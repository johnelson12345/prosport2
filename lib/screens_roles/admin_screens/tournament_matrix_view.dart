...
class _TournamentCalendarScreenState extends State<TournamentCalendarScreen>
  ...
  String _buildMatchVersusText(Map<String, dynamic> event) {
    ...
    if (!team1Display.contains('Winner') && !team1Display.contains('Loser') &&
      ...
      return 'Match $matchNumber: $team1Display vs $team2Display';
    }
    ...
    if (scores != null && scores.isNotEmpty) {
      ...
      if (scoreKeys.length >= 2) {
        ...
        if (!scoreTeam1.contains('Winner') && !scoreTeam1.contains('Loser') &&
          ...
          return 'Match $matchNumber: $scoreTeam1 vs $scoreTeam2';
        }
      }
    }
    
    // Try to get from team1Name/team2Name fields
    if (event['team1Name'] != null && event['team2Name'] != null) {
      ...
      if (!directTeam1.contains('Winner') && !directTeam1.contains('Loser') &&
        ...
        return 'Match $matchNumber: $directTeam1 vs $directTeam2';
      }
    }
    
    // For placeholder matches, show the placeholder text
    if (team1Display.isNotEmpty && team2Display.isNotEmpty) {
      return 'Match $matchNumber: $team1Display vs $team2Display';
    } else if (team1Display.isNotEmpty) {
      return 'Match $matchNumber: $team1Display vs TBD';
    } else if (team2Display.isNotEmpty) {
      return 'Match $matchNumber: TBD vs $team2Display';
    }
  ...
  Widget _buildEnhancedGridEventCard(
    Map<String, dynamic> event,
    Color sportColor,
    List<Map<String, dynamic>> allSchedules,
  ) {
    ...
    return Material(
      elevation: 6,
      shadowColor: sportColor.withOpacity(0.4),
      borderRadius: BorderRadius.circular(12),
      child: InkWell(
        onTap: widget.isEditable ? () => _editMatchDateTime(context, event, allSchedules) : null,
        borderRadius: BorderRadius.circular(12),
        child: Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(
          ...
          child: ClipRRect(
            ...
            child: Stack(
              children: [
                ...
                // Edit button overlay (if editable)
                if (widget.isEditable)
                  Positioned(
                    top: 4,
                    right: 4,
  ...
  Widget _buildGridInfoChip(IconData icon, String label, Color color) {
    return Container(
  ...
  Widget _buildEnhancedGridView(List<Map<String, dynamic>> schedules, List<String> sports) {
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Column(
          children: [
            _buildEnhancedHeaderSection(visibleSports, sportsWithMatches),
            Expanded(
              child: _buildEnhancedTimeGrid(
                timeSlots,
                rowHeight,
                visibleSports,
                calendarMap,
            ...
            _buildEnhancedFooterSection(visibleSports, sportsWithMatches),
          ],
        ),
      ),
    );
  ...
  Widget _buildEnhancedHeaderSection(List<String> visibleSports, Set<String> sportsWithMatches) {
    return Container(
      height: 100,
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            ...
            const Color(0xFF0B5EC9),
            const Color(0xFF0A4DB5),
          ],
        ),
        boxShadow: [
          BoxShadow(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final sportColor = _getSportColor(sport);
                final isHovered = _hoveredColumn == index;

                return MouseRegion(
                  onEnter: (_) => setState(() => _hoveredColumn = index),
                  onExit: (_) => setState(() => _hoveredColumn = -1),
                  child: AnimatedContainer(
                    ...
                    child: Column(
                      ...
                      children: [
                        _buildAnimatedSportIcon(sportColor, isHovered),
                        const SizedBox(height: 8),
                        Text(
                          sport.toUpperCase(),
                          style: TextStyle(
          ...
          _buildEnhancedColumnSelectorButton(),
        ],
      ),
    );
  }

  Widget _buildAnimatedSportIcon(Color color, bool isHovered) {
  ...
  Widget _buildEnhancedTimeHeader() {
    return Container(
      width: 100,
      decoration: BoxDecoration(
        border: Border(
  ...
  Widget _buildEnhancedTimeGrid(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, rowIndex) {
        ...
        final isHoveredRow = _hoveredRow == rowIndex;
        ...
        return MouseRegion(
          ...
          child: AnimatedContainer(
            ...
            child: Row(
              children: [
                _buildEnhancedTimeSlotCell(timeSlot, isEvenRow),
                Expanded(
                  child: ListView.builder(
                    scrollDirection: Axis.horizontal,
                    controller: _horizontalScrollController,
                    ...
                    itemBuilder: (context, colIndex) {
                      ...
                      final isHoveredCell = _hoveredRow == rowIndex && _hoveredColumn == colIndex;
                        ...
                        child: Stack(
                          ...
                          children: [
                            if (startingEvents.isEmpty && !isCovered)
                              _buildEnhancedEmptyCell(sportColor, isHoveredCell),
                            ...startingEvents.asMap().entries.map((entry) {
                              final eventIndex = entry.key;
                              final event = entry.value;
                              final eventId = event['id'];
                              final spanInfo = eventSpanMap[timeSlot]?[sport]?[eventId];
                              final span = spanInfo?['span'] ?? 1;

                              final double topOffset = eventIndex * 12.0;
                              final double leftOffset = eventIndex * 8.0;
                              final double rightOffset = eventIndex * 8.0;

                              return Positioned(
                                top: topOffset + 6,
                                left: leftOffset + 6,
                                right: rightOffset + 6,
                                height: (rowHeight * span) - 20 - topOffset,
                                child: TweenAnimationBuilder(
                                  duration: Duration(milliseconds: 300 + (eventIndex * 80)),
                                  ...
                                  builder: (context, value, child) {
                                    return Transform.scale(
                                      ...
                                      child: Opacity(
                                        opacity: value,
                                        child: _buildEnhancedGridEventCard(
                                          event,
                                          sportColor,
                                          schedules,
                                        ),
  ...
  Widget _buildEnhancedTimeSlotCell(String timeSlot, bool isEvenRow) {
    return Container(
      width: 100,
      decoration: BoxDecoration(
        border: Border(
  ...
  Widget _buildEnhancedEmptyCell(Color sportColor, bool isHovered) {
    return Center(
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 200),
        ...
        decoration: BoxDecoration(
          ...
          shape: BoxShape.circle,
          border: Border.all(
            color: sportColor.withOpacity(isHovered ? 0.3 : 0.1),
            width: 1,
          ),
        ),
        child: Center(
          child: Text(
            '—',
            style: TextStyle(
              color: sportColor.withOpacity(isHovered ? 0.5 : 0.3),
  ...
  Widget _buildEnhancedFooterSection(List<String> visibleSports, Set<String> sportsWithMatches) {
    return Container(
      height: 60,
      decoration: BoxDecoration(
        gradient: LinearGradient(
      ...
      child: Row(
        children: [
          ...
          _buildEnhancedScrollControls(),
        ],
      ),
    );
  }

  Widget _buildEnhancedScrollControls() {
    return Container(
      width: 50,
      alignment: Alignment.center,
      child: Row(
          ...
          IconButton(
            ...
            onPressed: () {
              _horizontalScrollController.animateTo(
                _horizontalScrollController.offset - 280,
                duration: const Duration(milliseconds: 300),
  ...
  Widget _buildEnhancedColumnSelectorButton() {
    return Container(
      width: 50,
      decoration: BoxDecoration(
        border: Border(
      ...
      child: Center(
        child: Tooltip(
          ...
          child: Material(
            color: Colors.transparent,
            child: InkWell(
              onTap: () => _showColumnSelectorDialog(_availableSports),
              borderRadius: BorderRadius.circular(30),
              child: Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
